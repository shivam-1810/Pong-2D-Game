...
public class PongTable extends SurfaceView implements SurfaceHolder.Callback {
    ...
    public void initPongTable(Context ctx,AttributeSet attr){
        ...
        mPlayer = new Player(playerPaint, racketHeight,racketWidth);
        ...
        mOpponent = new Player(opponentPaint, racketHeight, racketWidth);
    ...
    private void doAI(){
        ...
        if (mOpponent.bounds.top > mBall.cy){
            movePlayer(mOpponent,
                    ...
                    mOpponent.bounds.top - PHY_RACKET_SPEED);
    ...
    private void handleCollision(Player player,Ball ball){
        ...
        if (player == mPlayer){
            ...
            PHY_RACKET_SPEED = PHY_RACKET_SPEED * 1.05f;
        }
    }
    ...
    private void placePlayers(){
        mPlayer.bounds.offsetTo(2, (float) (mTableHeight - mPlayer.getRacketHeight()) /2);
        mOpponent.bounds.offsetTo(mTableWidth-mOpponent.getRacketWidth()-2,
                (float) (mTableHeight - mOpponent.getRacketHeight()) /2);
    }


    private void placeBall(){
        mBall.cx = (float) mTableWidth /2;
        mBall.cy = (float) mTableHeight /2;
